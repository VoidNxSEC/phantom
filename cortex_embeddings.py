...
class EmbeddingGenerator:
    ...
    def encode(self, texts: List[str], batch_size: int = BATCH_SIZE, show_progress: bool = False) -> np.ndarray:
        """
...
class FAISSVectorStore:
    ...
    def load(cls, filepath: Path) -> 'FAISSVectorStore':
        ...
        logging.info(f"Loaded vector store from {filepath} ({len(store.texts)} vectors)")
        return store
...
class NumpyVectorStore:
    ...
    def search(self, query_embedding: np.ndarray, top_k: int = 10) -> List[SearchResult]:
        ...
        emb_norms = all_embeddings / np.linalg.norm(all_embeddings, axis=1, keepdims=True)
...
class EmbeddingManager:
    ...
    def add_texts(
        ...
        embeddings = self.generator.encode(texts, batch_size=batch_size, show_progress=True)
    ...
    @classmethod
    def load(cls, filepath: Path, model_name: str = DEFAULT_MODEL, device: str = "cpu") -> 'EmbeddingManager':
        """Load vector store"""
