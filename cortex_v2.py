...
try:
    from cortex import (
        PromptBuilder, LlamaCppClient, MarkdownInsights,
...
class CortexV2Pipeline:
    ...
    def process_document(self, filepath: Path) -> DocumentInsights:
        ...
        self.console.print(f"    ✓ Created {len(chunks)} chunks ({stats['avg_tokens_per_chunk']:.0f} tokens avg)")
        ...
        if self.enable_vector_db and self.embedding_manager:
            ...
            self.embedding_manager.add_texts(chunk_texts, metadata=chunk_metadata, batch_size=32)
        ...
        if self.llm_client:
            ...
            self.console.print("  [dim]3/4 Skipping LLM classification (disabled)[/dim]")
            chunk_insights = []
        
        # Step 4: Aggregate insights
        self.console.print("  [yellow]4/4 Aggregating insights...[/yellow]")
        aggregated = self._aggregate_insights(chunk_insights) if chunk_insights else self._empty_insights()
    ...
    def _classify_chunks_parallel(self, chunks: List[Chunk], source_file: str) -> List[ChunkInsights]:
    ...
    def _classify_single_chunk(self, chunk: Chunk, source_file: str) -> Optional[ChunkInsights]:
        ...
        try:
            ...
            prompt = PromptBuilder.build_extraction_prompt(chunk.text, f"{source_file} [chunk {chunk.metadata.chunk_id}]")
...
def main():
    ...
    # Input/Output
    parser.add_argument('-i', '--input', required=True, help='Input markdown file or directory')
    ...
    parser.add_argument('--chunk-overlap', type=int, default=DEFAULT_CHUNK_OVERLAP, help='Overlap tokens')
    parser.add_argument('--chunk-strategy', choices=['recursive', 'sliding', 'simple'], default='recursive')
    
    # Embeddings
    parser.add_argument('--enable-vector-db', action='store_true', help='Enable vector database')
    ...
    # LLM
    parser.add_argument('--llamacpp-url', default='http://localhost:8080', help='LlamaCPP server URL')
    ...
    # Search mode
    parser.add_argument('--search', help='Semantic search query (requires existing vector DB)')
    ...
    if args.search:
        ...
        pipeline = CortexV2Pipeline.load_vector_db(Path(args.vector_db_path), **vars(args))
