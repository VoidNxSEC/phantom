...
class SentimentEngine:
    ...
    def __init__(self, use_spacy: bool = True):
        ...
        if self.use_spacy:
            if spacy is None:
                print("SpaCy library not found. Disabling SpaCy preprocessing.", file=sys.stderr)
                self.use_spacy = False
            else:
                try:
                    self.nlp = spacy.load("en_core_web_sm")
                except OSError:
                    print("SpaCy model 'en_core_web_sm' not found. Disabling SpaCy preprocessing.", file=sys.stderr)
                    self.use_spacy = False
...
def main():
    # Simple CLI test
    engine = SentimentEngine(use_spacy=False) # Default false to avoid model error if not present
